...
namespace DesafioFinancas360
{
    class Program
    {
        static List<char> Alfabeto = new List<char> { 'k', 'b', 'w', 'r', 'q', 'd', 'n', 'f', 'x', 'j', 'm', 'l', 'v', 'h', 't', 'c', 'g', 'z', 'p', 's' };
        static List<char> LetrasFoo = new List<char> { 's', 'l', 'f', 'w', 'k' };
        static List<char> LetrasBar = new List<char>();
        static List<char> NossoAlfabeto = new List<char> { 'a', 'b', 'c', 'd', 'e', 'f', 'g', 'h', 'i', 'j', 'k', 'l', 'm', 'n', 'o', 'p', 'q', 'r', 's', 't' };

        static void Main(string[] args)
        {
            //Método chamado para popular a lista LetrasBar com suas respectivas letras.
            MontaLetrasBar();

            //Captura do valor que o usuário irá digitar.
                ...
                if (texto.ToUpper() == "A" || texto.ToUpper() == "B")
                {
                    ...
                    foreach (var palavra in textoEscolhido.Split(' '))
                    {
                        ...
                        if (EhVerbo(palavra))
                        {
                            ...
                            //Aqui sabemos que a palavra é verbo, então verifica-se se o mesmo está na 1ª pessoa.
                            if (EhVerboPrimeiraPessoa(palavra))
                                verbosPrimeiraPessoaTexto++;
                        }

                        //Popula a lista com a palavra caso a mesma já não exista na lista, assim garante-se que não teremos palavras repetidas quando ordenarmos lexicograficamente.
                        if (!textoSemRepeticao.Contains(palavra))
                            textoSemRepeticao.Add(palavra);

                        //Popula a lista com a palavra traduzida no nosso alfabeto para ordenarmos a mesma lexicograficamente.
                        palavrasNossoAlfabeto.Add(TransformaNoNossoAlfabeto(palavra));
                    }

                    //Ordena as palavras lexicograficamente. 
                    palavrasNossoAlfabeto = palavrasNossoAlfabeto.OrderBy(p => p).ToList();

                    //Agora que temos a lista ordenada lexicograficamente, iremos traduzir de volta para o alfabeto Klingon. 
                    //Aproveitaremos para ao mesmo tempo, calcular o valor numérico da letra.
                    foreach (var palavra in palavrasNossoAlfabeto)
                    {
                        //Variaveis que serão utilizadas no calculo do valor numérico da palavra.
                        var valorNumerico = 0;
                        var valorBase = 1;

                        //Aqui percorreremos cada letra da palavra para traduzir para o alfabeto Klingon e calcular o valor numérico.
                        for (var j = 0; j < palavra.Length; j++)
                        {
                            //Letra no nosso alfabeto.
                            char letra = char.Parse(palavra.Substring(j, 1));

                            //Letra do nosso alfabeto traduzida para o alfabeto Klingon.
                            char letraAlfabetoKlingon = TransformaEmLetraAlfabetoKlingon(letra);

                            //Montando a frase novamente no alfabeto Klingon.
                            textoAlfabetoKlingon += letraAlfabetoKlingon;

                            //Calcula o valor numérico da letra e soma ao valor já acumulado da palavra.
                            valorNumerico += CalculaValorDaLetra(letraAlfabetoKlingon, valorNumerico, valorBase);
                            //Como é base 20, multiplica-se por 20.
                            valorBase *= 20;
                        }
                        //Quando acaba o for, temos a palavra montada. Aqui damos espaço para a próxima palavra não escrever junto com a atual.
                        textoAlfabetoKlingon += " ";
                        
                        //Verifica se o valor numérico desta palavra é um número bonito.
                        if (EhBonito(valorNumerico))
                            quantidadeBonitos++;
                    }

                    Console.WriteLine("\nA quantidade de preposições no Texto é: " + preposicoesTexto + ".");
                    Console.WriteLine("\nA quantidade de Verbos no Texto é: " + verbosTexto + " dos quais " + verbosPrimeiraPessoaTexto + " estão em primeira pessoa.\n");
                    Console.WriteLine(textoAlfabetoKlingon);
                    Console.WriteLine("\nA quantidade de números bonitos é: " + quantidadeBonitos);

                    Console.WriteLine("\nDigite -1 para sair ou qualquer tecla para repetir.");
                    texto = Console.ReadLine();
                }
                else
                {
                    Console.WriteLine("\nDigite -1 para sair ou qualquer tecla para tentar novamente.");
                    texto = Console.ReadLine();
                }
            }
            while (texto != "-1");
        ...
        /// <summary>
        /// Método responsável por verificar se uma palavra é preposição.
        /// </summary>
        /// <param name="palavra">Palavra que irá ser verificada.</param>
        /// <returns>Retorna verdadeiro se for preposição, caso contrário retorna false.</returns>
        static bool EhPreposicao(string palavra)
        {
            if (
                    palavra.Length == 3 &&
                    LetrasBar.Contains(char.Parse(palavra.Substring(2, 1))) &&
                    !palavra.Contains('d')
               )
                return true;
            else
        ...
        /// <summary>
        /// Método responsável por verificar se a palavra é verbo.
        /// </summary>
        /// <param name="palavra">Palavra que irá ser verificada.</param>
        /// <returns>Retorna verdadeiro se for verbo, caso contrário retorna false.</returns>
        static bool EhVerbo(string palavra)
        {
            if (
                    palavra.Length >= 8 &&
                    LetrasFoo.Contains(char.Parse(palavra.Substring(palavra.Length - 1, 1)))
               )
                return true;
            else
                return false;
        ...
        /// <summary>
        /// Método responsável por verificar se o verbo está na primeira pessoa.
        /// </summary>
        /// <param name="palavra">Palavra que irá ser verificada.</param>
        /// <returns>Retorna verdadeiro se estiver na primeira pessoa, caso contrário retorna false.</returns>
        static bool EhVerboPrimeiraPessoa(string palavra)
        {
            if (LetrasBar.Contains(char.Parse(palavra.Substring(0, 1))))
                return true;
            else
                return false;
        }
        ...
        /// <summary>
        /// Método responsável por popular a lista com as letrar do tipo Bar.
        /// </summary>
        static void MontaLetrasBar()
        {
            foreach (var letra in Alfabeto)
            {
                if (!LetrasFoo.Contains(letra))
                    LetrasBar.Add(letra);
            }
        }

        /// <summary>
        /// Método responsável por traduzir uma palavra do alfabeto Klingon no nosso alfabeto.
        /// </summary>
        /// <param name="palavra">Palavra no alfabeto Klingon.</param>
        /// <returns>Retorna a palavra no nosso alfabeto.</returns>
        static string TransformaNoNossoAlfabeto(string palavra)
        {
            string textoNoNossoAlfabeto = string.Empty;
            ...
            for (var j = 0; j < palavra.Length; j++)
            {
                ...
                textoNoNossoAlfabeto += NossoAlfabeto[posicao];
            }

            return textoNoNossoAlfabeto;
        }

        /// <summary>
        /// Método responsável por traduzir uma letra do nosso alfabeto, na letra correspondente do alfabeto Klingon.
        /// </summary>
        /// <param name="letra">Letra a ser traduzida.</param>
        /// <returns>Retorna a letra traduzida no alfabeto Klingon.</returns>
        static char TransformaEmLetraAlfabetoKlingon(char letra)
        {
            int posicao = NossoAlfabeto.IndexOf(letra);

            return Alfabeto[posicao];
        }

        /// <summary>
        /// Método responsável por transformar a letra em número e após isso calcular o valor da palavra.
        /// </summary>
        /// <param name="letra">Letra a ser transformada em número.</param>
        /// <param name="valorNumerico">Valor acumulado e que irá ser somado com o valor transformado da letra.</param>
        /// <param name="valorBase">Valor que determina se está em unidade, dezena, centena...</param>
        /// <returns>Retorna o valor acumulado da palavra.</returns>
        static int CalculaValorDaLetra(char letra, int valorNumerico, int valorBase)
        {
            int posicao = Alfabeto.IndexOf(letra);

            return valorNumerico += posicao * valorBase;
        }

        /// <summary>
        /// Método responsável por verificar se um número é bonito.
        /// </summary>
        /// <param name="numero">Número a ser verificado.</param>
        /// <returns>Retorna verdadeiro se for um número bonito, caso contrário retorna false.</returns>
        static bool EhBonito(int numero)
        {
